...
def visualize_trajectory_2d(new_map,pose,path_name="trajectory",show_ori=False):
  '''
  ...
  n_pose = pose.shape[2]
  ax.plot(pose[0,3,:],pose[1,3,:],'r-',label=path_name) #x,y in each 4*4 matrix
  ax.scatter(new_map[0],new_map[1],marker = '.', s = 5, label="landmarks")
