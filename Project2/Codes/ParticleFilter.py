...
class particle_filter():
    def __init__(self, number = 10, noise_x = 1, noise_y = 1, noise_theta = 1, Neff = 2, xmin = 0, ymin = 0, res = 0):
    ...
    def weight_update(self,data_bodyF,map):
        cr = np.zeros(shape=(self._particleNumber, 1))
        index = 0
        for particle in self._particle:
            Rotation_matrix = compute_rotation_matrix(wx=0, wy=0, wz=particle[2])
            Rotation_matrix = Rotation_matrix[0:2].T
            Rotation_matrix = Rotation_matrix[0:2].T
            particle_x = particle[0]  # x
            particle_y = particle[1]  # y
            data_worldF = Rotation_matrix.dot(data_bodyF)
            world_x = data_worldF[0]
            max_x = max(world_x)
            min_x = min(world_x)
            world_y = data_worldF[1]
            max_y = max(world_y)
            min_y = min(world_y)
            c = self.mapCorrelation(map,[min_x,max_x],[min_y,max_y],data_worldF,particle_x,particle_y)
            cr[index] = c
            index += 1
        #update weight
        s_cr = np.exp(cr-np.mean(cr))
        sum_cr = self._proba @ (s_cr)
        for i in range(0,self._particleNumber):
            self._proba[i] = self._proba[i] * s_cr[i] / sum_cr
        #print(self._proba)
        #print("proba:", self._proba)
        Neff = 1 / np.sum(np.square(self._proba))
        particle_list = np.arange(0,self._particleNumber)
        if Neff <= self.__Neff:
            particle_list = np.random.choice(particle_list, self._particleNumber, p = self._proba)
            self._particle = np.array([self._particle[particle_list]])[0]
            self._proba = np.array([1/self._particleNumber]*self._particleNumber)
            self.__update += 1

    def mapCorrelation(self, im, x_im, y_im, vp, xs, ys):
        xmin = x_im[0]
        xmax = x_im[-1]
        x_drift = np.int16((xmin - self.__xmin) / self.__res)
        nx = np.int16(np.round((xmax - xmin) / self.__res)) + x_drift
        ymin = y_im[0]
        ymax = y_im[-1]
        y_drift = np.int16((ymin - self.__ymin) / self.__res)
        ny = np.int16(np.round((ymax - ymin) / self.__res)) + y_drift
        y1 = vp[1] + ys  # 1 x 1076
        iy = np.int16(np.round((y1 - ymin) / self.__res)) + y_drift
        x1 = vp[0] + xs  # 1 x 1076
        ix = np.int16(np.round((x1 - xmin) / self.__res)) + x_drift
        valid = np.logical_and(np.logical_and((iy >= 0), (iy < ny)), \
            np.logical_and((ix >= 0), (ix < nx)))
        cpr = np.sum(im[iy[valid], ix[valid]])
        return cpr
...
class Map:
    def __init__(self, Lidar_data, Joint_data, value = 0, threshold_1 = 0, threshold_2 = 0, satur_u = 0, satur_l = 0):
        ...
        self.__record = np.zeros([len(Lidar_data),6]).astype(float)
    def Map_init(self, size, resolution):
        ...
        self._particles = particle_filter(100, 0.01, 0.01, 0.01, 1.6, self.__MAP['xmin'],self.__MAP['ymin'],self.__MAP['res'])
    def load_data(self):
    ...
    def update_par(self):
        self._particles.motion_update(self._lidar['delta_pose'])
        self.__iteration += 1
    ...
    def Transform(self, z_min,z_max):
        ...
        particle_position = self._particles.Prob_particle() #obtain the position of the most probably particle to update the map
        self.__record[self.__iteration, 0] = self._lidar["ts"]
        self.__record[self.__iteration, 1] = particle_position[0]
        self.__record[self.__iteration, 2] = particle_position[1]
        self.__record[self.__iteration, 3] = particle_position[2]
        self.__record[self.__iteration, 4] = neck_angle
        self.__record[self.__iteration, 5] = head_angle
        ...
        self._lidar['coordinate_b'] = Rotation_matrix.dot(self._lidar['coordinate'])
        self._lidar['coordinate_b'][2] += 0.33
        z_ground = -0.93 #contact points with the ground
        indValid = self._lidar['coordinate_b'][2] > z_ground
        self._lidar['coordinate_b'][0] = self._lidar['coordinate_b'][0][indValid] #x
        self._lidar['coordinate_b'][1] = self._lidar['coordinate_b'][1][indValid] #y
        self._lidar['coordinate_b'][2] = self._lidar['coordinate_b'][2][indValid] #z
        if self.__iteration > 0:
            self._particles.weight_update(self._lidar['coordinate_b'][0:2],self.__MAP['map']) #update particle weights
        Rotation_matrix = compute_rotation_matrix(wx = 0, wy = 0, wz = angle_w2b)
        self._lidar['coordinate_w'] = Rotation_matrix.dot(self._lidar['coordinate_b'])
        self._lidar['x'] = self._lidar['coordinate_w'][0]
        self._lidar['y'] = self._lidar['coordinate_w'][1]
        xs0 = self._lidar['x'].squeeze()
        ys0 = self._lidar['y'].squeeze()
        ...
        self.__MAP['map'][np.logical_and(self.__MAP['Probability'] > 0, self.__MAP['map'] != 2)] = 1
        self.__MAP['map'][np.logical_and(self.__MAP['Probability'] < 0, self.__MAP['map'] != 2)] = 0
    ...
    '''
    def draw_map(self):
         print("During all the iterations, the resampling time is:",self._particles.print_update())
    ...
    def save_map(self, data_i):
        ...
        np.save("motion_data"+str(data_i)+".npy",self.__record)
if __name__ == "__main__":
    ...
    for data_i in range(0,5):
        ...
        r0 = get_rgb("cam/RGB_0") #for coloring the map
        d0 = get_depth("cam/DEPTH_0") #for coloring the map
