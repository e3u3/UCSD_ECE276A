...
def mapCorrelation(im, x_im, y_im, vp, xs, ys, xresolution, yresolution):
  xmin = x_im[0]
  x_drift = (xmin + 20) / xresolution
  xmax = x_im[-1]
  nx = np.int16(np.round((xmax-xmin)/xresolution))
  ymin = y_im[0]
  y_drift = (ymin + 20) / xresolution
  ymax = y_im[-1]
  ny = np.int16(np.round((ymax-ymin)/yresolution))
  y1 = vp[1] + ys # 1 x 1076
  ...
  #print("111:",len(im[ix[valid],iy[valid]]))
  return cpr
...
def test_mapCorrelation():
  ...
  x_im = np.arange(MAP['xmin'],MAP['xmax']+MAP['res'],MAP['res']) #x-positions of each pixel of the map in physical unit
  y_im = np.arange(MAP['ymin'],MAP['ymax']+MAP['res'],MAP['res']) #y-positions of each pixel of the map in physical unit

  x_range = np.arange(-0.2,0.2+0.05,0.05)
  y_range = np.arange(-0.2,0.2+0.05,0.05)
  
  print("Testing map_correlation...")
  c = mapCorrelation(MAP['map'],x_im,y_im,Y[0:3,:],x_range[4:5],y_range[4:5])
  print("c:",c)
  print("map:",MAP['map'])
  print("Y:",Y[0:3,:])
  print("x:",x_range)
